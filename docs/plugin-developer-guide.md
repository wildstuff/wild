...
repository, or the published templates under `templates/` for
...
`examples/tool-providers/cash-forecast/wit/deps/` is the
...
- **Semantic / hybrid:** `find-by-hybrid(type, field, text-query, query-vec, k,
  ...
  vector** — so also import `wild:ai/embed@0.5.0`, embed the query text
...
`plugins/tools/m365-mail-connector/sidecar.template.json`
(settings, auth_bindings, capability_bundles — all in one file; the
SharePoint connector is a forged template now, ADR-0343, and carries its
contract in `template.json` instead).
...
**What kind of credential it is** rides as `credential` (ADR-0343 D1), a
closed set tagged on `kind`: `{"kind":"bearer"}` · `{"kind":"header",
"name":"X-Api-Key"}` · `{"kind":"basic"}` · `{"kind":"query","param":
"api_key"}` · `{"kind":"path","segment":"WILDBOTTOKEN"}` · `{"kind":"oauth2",
"grant":"client_credentials"|"authorization_code"}` (an `oauth2` credential
also ships its `oauth` template; its token rides as a bearer). The older
flat string `inject` (`bearer | basic | header:<Name> | query:<key> |
path:<sentinel>`) is still read — a declaration that says neither is a
bearer token, decided in ONE place (`CredentialKind::from_legacy`) — but a
new declaration says `credential`; a `credential` beside an `inject` that
disagrees is refused, and so is an `inject` string outside that vocabulary
(it used to read silently as `bearer`). A connector's `source_connector.setup`
is DERIVED from these bindings and cross-checked at load: declare the
bindings and let `setup` follow. Note the field is new (2026-09): a daemon
that predates it refuses a sidecar carrying it (`deny_unknown_fields`), so
bundled plugins convert on touch and templates ship it once their readers
have moved.
...
Published references you can open from a public clone: the
ADR-0202 Procedure trio
`examples/tool-providers/{cash-forecast,fx-exposure,payment-delay}`
— `cash-forecast` carries the richest surface of the three (procedure,
skills and training beside its `sidecar.json`) — then
`templates/slugify/` for a shipped template's own source mirror,
`examples/consumers/embed-consumer/` for a source-free
...
`m365-mail-connector/skills/m365-mail-setup.md` (OAuth app registration +
admin consent),
...
### What a tool reply carries — two halves, one form

A tool reply is read by two audiences at once, and only one of them can
be told apart by a reader scanning key/value pairs. Keep them apart in
the reply itself:

- the **operator half** — `status`, `note`, and the named facts. `note`
  carries the finished operator sentence: the half meant to be read
  aloud, in the accountant's vocabulary.
- the **`audit` half** — a sub-object holding what the machine
  determined and the operator can do nothing with: ids, digests, image
  references.

```rust
Ok(tool_result::with_audit(
    json!({
        "status": "awaiting-consent",
        "offering": offer.name,
        "note": format!("`{}` is loaded and its signature checks out. …", name),
    }),
    json!({ "escalation_id": escalation_id, "digest": digest }),
)
.to_string())
```

`with_audit` removes every key the audit half names from the operator
half, so the split holds even at a door that names a handle in both — a
door that names a handle beside the prose has not decided to show it, it
has forgotten to place it.

**Why.** Measured on walk-7 (2026-09-09): the reply confirming a loaded
offering handed the operator
`escalation-id: template-build-a7e4330393f5d505`. The id was not wrong
to be in the reply — the run needs it — but sitting flat beside the
sentence, nothing distinguished it from something an accountant was
meant to read.

**Adopted on touch, not swept.** Move a door's reply to this form the
next time you edit it. And the honest limit, because it decides what you
may claim in a PR: this is a witnessable substrate, not an enforcement.
Nothing in it stops a model from quoting a field it can see; what it
does is stop the reply from *inviting* the quote, and make the split
...
A plugin's version is written in up to **eight** places, and
`scripts/component-shape.lock` makes nine. One of them is the source
(`plugins/<kind>/<dir>/Cargo.toml`); the rest are derived, and each is
checked by a gate that fails the build when it lags. Do not move them by
hand — there is a writer, and it knows the order.

```
python3 scripts/ci/check-bundled-plugin-inventory-version-consistency.py \
    --set <slug>=<version>
```

It moves the source and then every representation the check would
otherwise refuse: the sidecar template (its `version` field, its OCI ref,
and the version inside `wasm_filename` + `source.path`), the built-in
component-type YAML, `embedded_bootstrap.yaml`, both marketplace feeds,
the operator-facing inventory table, and the plugin's own `Cargo.lock`.
The writer and the check share one map of where those live, on purpose —
a second copy of "where the versions are" is the drift the gate exists to
stop. The slug is the one the GATE reports, not the directory name; the
two differ exactly where plugins share a directory name (`llama-embed`
and `llama-rerank` both live at `plugins/*/llama`).

Three steps remain afterwards, and the writer prints them in the order
that works — each changes the component's own-tree hash, so re-pinning
before they are done records a half-state:

1. `cargo run -p xtask -- oci-build-component --target plugins/<kind>/<dir>`
2. `cargo nextest run -p wild-host --test integration -E 'test(every_fixture_manifest_states)'`
   — it names any hand-written fixture left behind, with both versions.
3. `cargo run -p xtask -- component-shape-check --write` — **last**.

**On step 3, measured 2026-09-11:** `--write` re-pins EVERY component, not
only yours. Most of the extra lines are `full`-only moves — the open half
ADR-0223 reports rather than refuses — and they belong to whatever else
has moved under the tree since the last pin. Committing them silences that
report for everyone. Keep your own line and restore the rest
(`git checkout -- scripts/component-shape.lock`, then re-apply the one
line) unless you mean to adopt the others.

**Why this section exists.** The remedy was already printed — by
`oci-version-discipline` and by `component-shape-check`, on the path where
a gate CATCHES a stale version. An author who bumps deliberately, before
any gate complains, never sees it: they edit `Cargo.toml`, grep for the
old string, and chase the other seven by hand. That happened twice in one
day (#7811, #7817) before this was written down.
