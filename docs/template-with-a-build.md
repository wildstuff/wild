...
|---|---|
| `template.json` | the manifest — name, version, flavor, WIT baseline, and the artefact's declaration. The version is not typed here: the packer derives it from the packed `Cargo.toml`, the number the compiled component reports at runtime — see below |
...
`templates/<name>/`, so the source can be browsed and copied
...
A **connector** template carries one more: `source_connector`, its declared
contract (setup × structure × binding × scope) — the block the operator-spec
declares under `#### Source-Connector:` and the forge writes into the
folder's build record. The packer derives it from that record the way it
derives `built_in`, and refuses a hand-written copy that disagrees; the
installing daemon carries it on the build so the registrar reads the contract
beside the `source.connector` capability, which it refuses on its own.
...
oras pull ghcr.io/wildstuff/templates/slugify:0.1.3 -o pkg/
...
sig = open("pkg/slugify-0.1.3.sig", "rb").read()
VerifyKey(key).verify(open("pkg/slugify-0.1.3.tar.gz", "rb").read(), sig)
print("signature OK")
PY
```
...
2. **Pack the folder:**
   ...
   **To move the template's version, bump `[package].version` in the
   component's `Cargo.toml`** and leave `version` out of the on-disk
   `template.json`. The packer writes it into the packed manifest from
   there, because that is the number the compiled component reports at
   runtime (`env!("CARGO_PKG_VERSION")`). A `template.json` that states a
   different number is refused, naming both: the shipped `slugify-0.1.2`
   had been bumped in the manifest alone and told the daemon it was
   `0.1.0` (#7987).
...
## Upgrading from a bundled plugin

A profile that installed the bundled SharePoint plugin keeps running it after
the catalog row swaps to the template — nothing moves on its own. The road to
the template is the update road, in the operator's words, and it was walked
before the swap (ADR-0343 D8.5, #8099 rift 12):

1. **The check.** "Gibt es Updates für meine Bausteine?" — the connector is
   listed as updatable to the template's version (`0.1.1 → 0.2.0`), with the
   note that the update arrives as a marketplace template: built here, or its
   shipped bytes verified, then approved before it runs. A template offering is
   the update channel of the plugin it installs; the catalog row needs no
   `oci_ref` for that.
2. **The yes.** "Ja, aktualisier den SharePoint-Baustein." — the upgrade
   enters the template install road: the package is fetched and its signature
   verified, the contract is seeded beside the tribe's specs, the connector is
   built (or, with a nameable publisher, its shipped bytes are registered and
   reproduced in the background), and the row lands pending.
3. **The approval.** The card names the three tools, the egress and the
   OAuth binding; the operator's approval deploys it. The version floor keeps
   the profile from going backwards: the template must carry a higher version
   than the installed plugin, which is why the swap ships `0.2.0` above the
   bundled `0.1.1`.
4. **Secrets keep their names.** `sharepoint-client-secret` and the OAuth
   binding `sharepoint` are the same on both sides; nothing is re-entered.

What does NOT happen: no pull of a binary, no silent replacement, and no
second copy of the connector — the template registers under the slug the
plugin had.
