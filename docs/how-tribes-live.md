...
just an **inactive Tribe**: a charter on disk, no workers running,
no cost. There is no separate "idea" object to convert later; the
idea *is* the Tribe, asleep.

```mermaid
stateDiagram-v2
    [*] --> Inactive: you describe an idea<br/>to Elder (Intake)
    Inactive --> Inactive: shape & refine<br/>in conversation (Mentor)
    Inactive --> Active: activate<br/>(Chief spawns, cycles begin)
...
- **Inactive** — not started yet: the idea lives as a charter
  (`specs/sketch.md`) and costs nothing to keep. You and Elder can shape it for as
...
- **Pause / Archive** — freeze a running Tribe (it remembers its
  ...
  data stays on disk and can be resurrected. A paused Tribe is
  inactive too — the overview says so, and "paused" is the
  second word under it.
...
<a href="diagrams/how-tribes-live/scenes/02-lifecycle.svg"><img src="diagrams/how-tribes-live/scenes/02-lifecycle.png" alt="The life of a Tribe — inactive, shaped, active, paused" width="780"/></a>
...
> plant-growth chart. Stage 1: a seed in soil labelled "Inactive
...
**Day 0 — born (inactive).** You tell Elder: *"Keep an eye on three
...
timeline
    ...
    Day 0  : Born — inactive charter, ~zero cost
...
| **Blueprint / spec / sketch** | The Markdown charter a Tribe runs on; a pinned idea's is `specs/sketch.md`. | ADR-0034 · ADR-0042 |
...
| 2 | Life of a Tribe | Plant-growth strip: seed asleep → watered → sprout with campfire → small tree under a moon (inactive→shaped→active→paused). |
