...
OUTBOUND = [
    ...
    # RELATIVE, like every other open row — and for the same reason. These three
    # were written with absolute dates when the FX lane was added, which is the
    # defect the comment above records as already fixed for the block it sits
    # over: the clock walked two of them past their due date into the OVERDUE
    # set, and each one that lands there spends one of the four slots the
    # freshness ceiling leaves. Anchoring them puts the count back where the
    # hand-authored tail intends it.
    #
    # Staggered, too: RE-0050 used to share +20 with RE-0042, so a single day
    # moved the overdue count 8 -> 10 and skipped the ceiling entirely. Two
    # rows on one due date make the alarm unable to fire at its own bound.
    ("RE-2026-0050", "BP-005", TODAY - timedelta(days=33), TODAY + timedelta(days=24), "24000.00", None),
    ("RE-2026-0051", "BP-001", TODAY - timedelta(days=20), TODAY + timedelta(days=48), "12000.00", None),
    ("RE-2026-0052", "BP-004", TODAY - timedelta(days=17), TODAY + timedelta(days=76), "9500.00", None),
]
