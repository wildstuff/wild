...
- **`multi-invoice-scan-split`** — the full *Capture → Extract → Confirm → File*
  chain: `segment` (cut the scan where the invoice number changes, `strategy:
  by-field`) → an agentic `extract-worker` per invoice → a `confirm` pause (human
  in the loop) → a `store` sink-data. The manual sibling of the
  `invoices-inbound-pdf` connector takeover, but with an explicit operator
  confirm before filing. A scan with no readable invoice number falls back to
  the model, guided by the stage's `instruction` — and the run record says so.
