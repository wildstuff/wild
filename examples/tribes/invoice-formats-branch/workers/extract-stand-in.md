...
so the host's `ingest-candidates` sink admits them through the real govern
seam. The per-format `custom_instruction` is echoed back as `tuning_hint` so
the fixture can assert the branch tuning reached dispatch.
